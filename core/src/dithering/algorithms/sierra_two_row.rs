...
impl SierraTwoRow {
    pub const NAME: &'static str = "two_row_sierra";

    pub const fn definition() -> FilterDefinition {
        FilterDefinition {
            name: Self::NAME,
            params: <SierraTwoRowParams as crate::filter::FilterParams>::PARAMS,
        }
    }
}

impl DitherAlgorithm for SierraTwoRow {
    type Params = SierraTwoRowParams;
    ...
    fn apply(&self, image: &mut Image, palette: &Palette, params: &Self::Params) {
        ...
        // Sierra Two-Row taps relative offset (dx, dy) and weight
            ...
            for x in 0..width {
                ...
                for &(dx, dy, weight) in TAPS {
                    ...
                    if nx >= 0 && nx < width as isize && ny >= 0 && ny < height as isize {
                        ...
                        f_pixels[n_idx][0] += err_r * (weight / 16.0);
                        f_pixels[n_idx][1] += err_g * (weight / 16.0);
                        f_pixels[n_idx][2] += err_b * (weight / 16.0);
                    }
                }
