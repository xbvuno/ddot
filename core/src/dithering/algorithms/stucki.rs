...
impl Stucki {
    pub const NAME: &'static str = "stucki";

    pub const fn definition() -> FilterDefinition {
        FilterDefinition {
            name: Self::NAME,
            params: <StuckiParams as crate::filter::FilterParams>::PARAMS,
        }
    }
}

impl DitherAlgorithm for Stucki {
    type Params = StuckiParams;
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if y + 2 < height {
                    // (x - 2, y + 2) -> 1/42
                    if x >= 2 {
                        let n_idx = (y + 2) * width + (x - 2);
                        f_pixels[n_idx][0] += err_r * 1.0 * factor;
                        f_pixels[n_idx][1] += err_g * 1.0 * factor;
                        f_pixels[n_idx][2] += err_b * 1.0 * factor;
                    }
                    // (x - 1, y + 2) -> 2/42
                    if x >= 1 {
