...
impl Jjn {
    pub const NAME: &'static str = "jjn";

    pub const fn definition() -> FilterDefinition {
        FilterDefinition {
            name: Self::NAME,
            params: <JjnParams as crate::filter::FilterParams>::PARAMS,
        }
    }
}

impl DitherAlgorithm for Jjn {
    type Params = JjnParams;
    ...
    fn apply(&self, image: &mut Image, palette: &Palette, params: &Self::Params) {
        ...
        // JJN taps relative offset (dx, dy) and weight
            ...
            for x in 0..width {
                ...
                for &(dx, dy, weight) in TAPS {
                    ...
                    if nx >= 0 && nx < width as isize && ny >= 0 && ny < height as isize {
                        ...
                        f_pixels[n_idx][0] += err_r * (weight / 48.0);
                        f_pixels[n_idx][1] += err_g * (weight / 48.0);
                        f_pixels[n_idx][2] += err_b * (weight / 48.0);
                    }
                }
